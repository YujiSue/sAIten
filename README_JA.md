...
# sAIten
このリポジトリは、GoogleスプレッドシートとGoogleフォームを利用して、記述式の小テストの出題とAIによる採点、および採点結果のスコアとコメントを返却するまでの、一連の過程を自動かつ即時に行うためのファイルおよびスクリプトをまとめたものです。  
  
主な目的は、教員の負担を抑えながら、学生の理解度を日常的に確認するための小規模な記述式テストを、円滑に実施できるようにすることです。  
...
sequenceDiagram
    ...
    Manager->>Control: キーの登録、認証など（初期設定）
