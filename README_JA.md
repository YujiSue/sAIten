...
# sAIten
このリポジトリは、Google SpreadSheetとGoogleフォームを使って、記述式の小テストを出題し、AIで採点、スコアとコメントを返却する過程を自動かつ即時的に行うための、ファイルとスクリプトをまとめたものです。  
及落に関わるような重要な試験というよりは、日頃の理解度を試すための小規模な試験の円滑な実施を目的としています。  
  
大学など学生の数が教員に比べて多いとき、記述式テストの採点が間に合わないために、やむなく多肢選択問題のみ実施しているケースも多々あるかと思います。  
本システムを使えば、教員は問題文と重点的に理解しておいてほしいポイントを、あらかじめスプレッドシート上に記入することで、簡易に記述式テストを実施し、学生へフィードバックすることが出来ます。  
もちろん、AIの採点をそのまま学生に返すことに抵抗がある場合は、ヒトがチェックできるように、採点と結果の送信のタイミングを任意に調節することも可能です。  

2026年現在、学生側もAIを濫用して「それっぽい」文章を生成してくることが、もはや当然になりつつあります。  
本システムは、こうした背景の中にあって、効率よく課題を与えて「本当に学んでほしいポイント」を教員側がコントロールし、学習効果を高めるための有益システムであると期待しています。


# 実行に当たって必要なもの
* Google アカウント  
取得方法は[こちら](https://support.google.com/accounts/answer/27441?hl=ja&co=GENIE.Platform%3DDesktop&oco=0)  
※ 所属機関がGoogle Workspace for Educationの契約をしている場合は、xxxx@google.comのメールアドレスを保有していなくても、所属期間の発行するメールアカウントで利用できます。  
